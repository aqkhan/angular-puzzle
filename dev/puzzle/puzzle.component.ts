...
@Component({
    ...
    [ngStyle] = "{display: (name.value.length > 0) ? 'block' : 'none'}"
    >
        <h2>Unlock{{(switch1.value == switch1Number && switch2.value == switch2Number && switch3.value == switch3Number && switch4.value == switch4Number) ? 'ED' : ''}} the freak'in puzzle</h2>
    ...
    <h2 [ngStyle]="{display: switch1.value == switch1Number && switch2.value == switch2Number && switch3.value == switch3Number && switch4.value == switch4Number ? 'block' : 'none'}">Congratulations {{name.value}}! You freak'in did it</h2>
    `
})
